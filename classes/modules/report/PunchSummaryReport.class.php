	...
	protected function _getOptions( $name, $params = null ) {
		...
		switch ( $name ) {
			...
			case 'custom_columns':
				...
				$other_field_names = $oflf->getByCompanyIdAndTypeIdArray( $this->getUserObject()->getCompany(), [ 15, 20, 30 ], [ 15 => '', 20 => 'job_', 30 => 'job_item_' ] );
			...
			case 'column_format':
				...
				if ( is_array( $columns ) ) {
					foreach ( $columns as $column => $name ) {
						...
						} else if ( strpos( $column, 'total_punch' ) !== false || strpos( $column, 'total_tainted_punch' ) !== false || strpos( $column, 'quantity' ) ) {
		...
		if ( $plf->getRecordCount() > 0 ) {
			foreach ( $plf as $key => $p_obj ) {
				...
				if ( !isset( $this->tmp_data['punch'][$p_obj->getUser()][$p_obj->getColumn( 'punch_control_id' )] ) ) {
					...
					$this->tmp_data['punch'][$p_obj->getUser()][$p_obj->getColumn( 'punch_control_id' )] = [
