...
class Form1099MiscReport extends Report {
	...
	protected function _checkPermissions( $user_id, $company_id ) {
		if ( $this->getPermissionObject()->Check( 'report', 'enabled', $user_id, $company_id )
				&& $this->getPermissionObject()->Check( 'report', 'view_form1099misc', $user_id, $company_id ) ) {
			return true;
		}
	...
	function getF1099MiscObject() {
		if ( !isset( $this->form_obj['f1099m'] ) || !is_object( $this->form_obj['f1099m'] ) ) {
									...
									if ( $tax_withheld_amount > 0 || in_array( $user_id, (array)$cd_obj->getUser() ) ) {
										...
										if ( $cd_obj->getCalculation() == 200 && $cd_obj->getProvince() != '' ) {
											...
											foreach ( range( 'a', 'z' ) as $z ) {
												...
												if ( !( isset( $this->tmp_data['state_ids'][$user_id]['l16' . $z] ) && isset( $this->tmp_data['state_ids'][$user_id]['l17' . $z . '_state'] ) && $this->tmp_data['state_ids'][$user_id]['l17' . $z . '_state'] != $cd_obj->getProvince() ) ) {
													$state_id = $z;
													break;
												}
											}
											...
											if ( $is_active_date == true ) {
												if ( !isset( $this->tmp_data['pay_stub_entry'][$user_id][$date_stamp]['l18' . $state_id] ) || ( isset( $this->tmp_data['pay_stub_entry'][$user_id][$date_stamp]['l18' . $state_id] ) && $this->tmp_data['pay_stub_entry'][$user_id][$date_stamp]['l18' . $state_id] == 0 ) ) {
											...
											$this->tmp_data['pay_stub_entry'][$user_id][$date_stamp]['l16' . $state_id] = bcadd( $this->tmp_data['pay_stub_entry'][$user_id][$date_stamp]['l16' . $state_id], Misc::calculateMultipleColumns( $data_b['psen_ids'], [ $cd_obj->getPayStubEntryAccount() ] ) );
					...
					foreach ( $user_rows as $user_id => $row ) {
						...
						if ( $ulf->getRecordCount() == 1 ) {
							...
							foreach ( range( 'a', 'z' ) as $z ) {
								...
								if ( ( ( isset( $row['l16' . $z] ) && $row['l16' . $z] > 0 ) || ( isset( $row['l18' . $z] ) && $row['l18' . $z] > 0 ) )
										&& ( isset( $row['l17' . $z . '_state'] )
												&& isset( $this->form_data['remittance_agency'][$legal_entity_id][$row['l17' . $z . '_state']] )
												&& isset( $this->form_data['remittance_agency_obj'][$this->form_data['remittance_agency'][$legal_entity_id][$row['l17' . $z . '_state']]] )
												&& $this->form_data['remittance_agency_obj'][$this->form_data['remittance_agency'][$legal_entity_id][$row['l17' . $z . '_state']]]->getType() == 20 ) ) {
									...
									if ( isset( $ee_data['l17' . $z . '_state_id'] ) && $ee_data['l17' . $z . '_state_id'] != '' ) {
