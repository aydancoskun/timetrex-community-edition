...
class Form940Report extends Report {
	...
	function _handlePayStubEntryRecord( $user_id, $date_stamp, $row, $setup_data, $form_data, $payments_over_cutoff, $before_adjustment_tax_rate, $excluded_wage_avg = 0, $state_credit_reduction_rates = null ) {
		...
		if ( isset( $form_data['state_total_payments'] ) ) {
			foreach ( $form_data['state_total_payments'] as $state => $state_psen_ids ) {
				...
				if ( $state_deducted_amount > 0 ) {
					...
					if ( isset( $state_credit_reduction_rates[$state] ) && $state_credit_reduction_rates[$state] != 0 ) {
		...
		if ( isset( $setup_data['line_10'] ) && $setup_data['line_10'] > 0 ) {
		...
		if ( !isset( $this->form_data['pay_period'][$legal_entity_id][$quarter_month][$date_stamp] ) ) {
			$this->form_data['pay_period'][$legal_entity_id][$quarter_month][$date_stamp] = Misc::preSetArrayValues( [], [ 'total_payments', 'exempt_payments', 'excess_payments', 'taxable_wages', 'before_adjustment_tax', 'adjustment_tax', 'after_adjustment_tax' ], 0 );
		}
	...
	function _getData( $format = null ) {
				...
				if ( $ra_obj->getStatus() == 10 &&
					...
					if ( $cdlf->getRecordCount() > 0 ) {
						foreach ( $cdlf as $cd_obj ) {
							if ( $cd_obj->getCalculation() == 15 && stripos( $cd_obj->getName(), 'unemployment' ) !== false ) { //15=Advanced Percent
		...
		$this->tmp_data['pay_stub_entry'] = []; //Reset this array once all YTD totals are calculated
		...
		if ( isset( $this->form_data['total'] ) ) {
			foreach ( $this->form_data['total'] as $legal_entity_id => $legal_entity_data ) {
				...
				if ( isset( $this->form_data['remittance_agency_states'][$legal_entity_id] ) ) {
					...
					if ( isset( $this->form_data['state'] ) && count( $this->form_data['state'] ) > 1 ) {
...
//					$f940->l11 = $f940sa->calcTotal();
				...
				if ( isset( $this->form_data ) && count( $this->form_data ) >= 6 ) {
					...
					$f940->l13 = ( isset( $setup_data['tax_deposited'] ) && $setup_data['tax_deposited'] != '' ) ? $setup_data['tax_deposited'] : null;
