	...
	protected function _getOptions( $name, $params = null ) {
		...
		switch ( $name ) {
			...
			case 'custom_columns':
				...
				$other_field_names = $oflf->getByCompanyIdAndTypeIdArray( $this->getUserObject()->getCompany(), [ 10, 4, 5, 12, 20, 30 ], [ 10 => '', 4 => 'branch_', 5 => 'department_', 12 => 'user_title_', 20 => 'job_', 30 => 'job_item_' ] );
			...
			case 'column_format':
				...
				if ( is_array( $columns ) ) {
					foreach ( $columns as $column => $name ) {
						...
						} else if ( strpos( $column, 'labor_burden_percent' ) !== false ) {
			...
			case 'aggregates':
				...
				if ( is_array( $dynamic_columns ) ) {
					foreach ( $dynamic_columns as $column ) {
						switch ( $column ) {
							default:
								if ( strpos( $column, 'hourly_rate' ) !== false || strpos( $column, 'wage' ) !== false || strpos( $column, 'labor_burden_percent' ) !== false ) {
									$retval[$column] = 'avg';
								} else {
									$retval[$column] = 'sum';
								}
