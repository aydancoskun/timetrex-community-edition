...
class AccrualPolicyFactory extends Factory {
	...
	function isInitialApplyFrequencyWindow( $current_epoch, $offset, $pay_period_dates = null, $u_obj = null ) {
		$apply_frequency_dates = $this->getApplyFrequencyWindowDates( $current_epoch, $offset, $pay_period_dates, $u_obj );
		if ( isset( $apply_frequency_dates['start_date'] ) && isset( $apply_frequency_dates['end_date'] ) ) {
			if ( is_object( $u_obj ) && TTDate::getMiddleDayEpoch( $u_obj->getHireDate() ) >= TTDate::getMiddleDayEpoch( $apply_frequency_dates['start_date'] ) && TTDate::getMiddleDayEpoch( $u_obj->getHireDate() ) <= TTDate::getMiddleDayEpoch( $apply_frequency_dates['end_date'] ) ) {
	...
	function calcAccrualPolicyTime( $u_obj, $epoch, $offset, $pps_obj, $pay_period_arr, $accrual_balance, $update_records = true ) {
		...
		if ( $u_obj->getStatus() == 10
				...
				&& ( $this->getMinimumEmployedDays() == 0
						|| TTDate::getDays( ( TTDate::getMiddleDayEpoch( $epoch ) - TTDate::getMiddleDayEpoch( $u_obj->getHireDate() ) ) ) >= $this->getMinimumEmployedDays() ) ) {
			...
			if ( $this->getType() == 30 ) {
				...
				if ( $in_apply_frequency_window == false
						&& $this->getEnableOpeningBalance() == true
						&& TTDate::getMiddleDayEpoch( $u_obj->getHireDate() ) == TTDate::getMiddleDayEpoch( $epoch )
					...
					if ( $accrual_balance < $milestone_obj->getMaximumTime() ) {
						...
						if ( $this->getType() == 20 //Calendar based only
								...
								(
										...
										||
										( $this->getEnableOpeningBalance() == true && $this->getEnableProRateInitialPeriod() == true && TTDate::getMiddleDayEpoch( $u_obj->getHireDate() ) == TTDate::getMiddleDayEpoch( $epoch ) )
								)
						) {
