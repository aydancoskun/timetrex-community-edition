			...
			foreach( $rslf as $rs_obj ) {
				...
				if ( $sf->isValid() ) {
					...
					if ( $schedule_result == TRUE
							...
							AND $rs_obj->getStatus() == 10 ) { //Make sure they are working for Auto-fill to kickin.
						Debug::text('Schedule has AutoFill enabled!', __FILE__, __LINE__, __METHOD__, 10);

						$commit_punch_transaction = FALSE;

						Debug::text('Punch In: '. TTDate::getDate('DATE+TIME', $rs_obj->getStartTime() ), __FILE__, __LINE__, __METHOD__, 10);
