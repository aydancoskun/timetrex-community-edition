				...
				$transaction_function = function () use ( $row, $validate_only, $ignore_warning, $validator_stats, $validator, $save_result, $key, $permission_children_ids ) {
					...
					if ( $is_valid == true ) { //Check to see if all permission checks passed before trying to save data.
						...
						if ( $lf->isNew() == true ) {
							...
							if ( isset( $udf_obj ) && is_object( $udf_obj ) ) {
								if ( !isset( $row['permission_control_id'] ) && $udf_obj->getPermissionControl() != TTUUID::getZeroID() && $udf_obj->getPermissionControl() != $udf_obj::getNotExistID() ) {
									Debug::Text( 'Using default permissions...', __FILE__, __LINE__, __METHOD__, 10 );
									$lf->setPermissionControl( $udf_obj->getPermissionControl() );
								}

								if ( !isset( $row['pay_period_schedule_id'] ) && $udf_obj->getPayPeriodSchedule() != TTUUID::getZeroID() && $udf_obj->getPayPeriodSchedule() != $udf_obj::getNotExistID() ) {
