...
class TTDate {
	...
	public static function convertSecondsToHMS( $seconds, $include_seconds = false ) {
		...
		( //Check if we're passed a numeric string value that is greater than PHP_INT_MAX.
				...
				&&
				(
						( $negative_number == false && bccomp( $seconds, PHP_INT_MAX, 0 ) === 1 )
						||
						( $negative_number == true && bccomp( $seconds, ( PHP_INT_MAX * -1 ), 0 ) === -1 )
				)
		)
		) {
	...
	public static function isMidnight( $epoch ) {
		if ( TTDate::getHour( $epoch ) == 0 && TTDate::getMinute( $epoch ) == 0 && TTDate::getSecond( $epoch ) == 0 ) {
			return true;
		}
