...
class CalculatePayStub extends PayStubFactory {
	...
	function getDeductionObjectArrayForSorting( $obj ) {
		...
		if ( is_a( $obj, 'UserDeductionFactory' ) ) {
			...
			return $arr;
		} else if ( is_a( $obj, 'PayStubAmendmentFactory' ) ) {
			$arr['type'] = get_class( $obj );
			$arr['obj_id'] = $obj->getId();
			$arr['id'] = substr( $arr['type'], 0, 1 ) . $obj->getId();
			...
			return $arr;
		} else if ( is_a( $obj, 'UserExpenseFactory' ) && is_object( $obj->getExpensePolicyObject() ) && is_object( $obj->getExpensePolicyObject()->getPayStubEntryNameObject() ) ) {
			$arr['type'] = get_class( $obj );
			$arr['obj_id'] = $obj->getId();
			$arr['id'] = 'E' . $obj->getId();
			$arr['name'] = '';
					...
					if ( $ud_obj->getCompanyDeductionObject()->getStatus() == 10 ) {
						...
						if ( is_array( $deduction_order_arr[$global_id] ) ) {
							...
							if ( $ud_obj->getCompanyDeductionObject()->isLookbackCalculation() == true ) {
								$dependency_tree->addNode( $global_id, [], [], $deduction_order_arr[$global_id]['order'] );
							} else {
	...
	function calculate() {
		...
		if ( ( $this->getEnablePostTerminationCalculation() == true && $this->getUserObject()->getTerminationDate() < $this->getPayPeriodObject()->getStartDate() ) ) {
		...
		if ( $this->getUserObject()->getStatus() != 10
				&& ( ( $is_termination_pay_period == false && $is_post_termination_pay_period == false ) &&
						( $this->getUserObject()->getTerminationDate() == '' || $this->getUserObject()->getTerminationDate() < $this->getPayPeriodObject()->getStartDate() ) )
		) {
		...
		if ( $this->getTransactionDate() != false && $this->getType() != 10 ) {
		...
		if ( $this->getEnableCorrection() == false && ( $this->getUserObject()->getHireDate() != '' && TTDate::getMiddleDayEpoch( $this->getUserObject()->getHireDate() ) > TTDate::getMiddleDayEpoch( $this->getPayPeriodObject()->getEndDate() ) ) ) {
			...
			foreach ( $deduction_order_arr as $calculation_order => $data_arr ) {
				...
				if ( isset( $data_arr['obj'] ) && is_object( $data_arr['obj'] ) ) {
					if ( $data_arr['type'] == 'UserDeductionListFactory' ) {
						...
						if ( $this->getUserObject()->getBirthDate() == '' && $ud_obj->getCompanyDeductionObject()->isUserAgeEligibility() == true ) {
						...
						if ( $this->getUserObject()->getHireDate() == '' && $ud_obj->getCompanyDeductionObject()->isUserLengthOfServiceEligibility() == true ) {
							$pay_stub->Validator->isTRUE( 'hire_date',
														  false,
						...
						if ( $ud_obj->getCompanyDeductionObject()->isActiveDate( $ud_obj, $pay_stub->getPayPeriodObject()->getEndDate(), $pay_stub->getTransactionDate() ) == true
								&& $ud_obj->getCompanyDeductionObject()->isActiveLengthOfService( $ud_obj, $pay_stub->getPayPeriodObject()->getEndDate(), $pay_stub->getPayPeriodObject()->getStartDate() ) == true
								&& $ud_obj->getCompanyDeductionObject()->isActiveUserAge( $this->getUserObject()->getBirthDate(), $pay_stub->getPayPeriodObject()->getEndDate(), $pay_stub->getTransactionDate() ) == true
								&& $ud_obj->getCompanyDeductionObject()->inApplyFrequencyWindow( $pay_stub->getPayPeriodObject()->getStartDate(), $pay_stub->getPayPeriodObject()->getEndDate(), $this->getUserObject()->getHireDate(), $this->getUserObject()->getTerminationDate(), $this->getUserObject()->getBirthDate() ) == true
