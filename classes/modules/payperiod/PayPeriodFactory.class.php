...
class PayPeriodFactory extends Factory {
	...
	*/
	...
	function Validate( $ignore_warning = true ) {
		...
		if ( $this->getDeleted() == false && ( $this->getStartDate() != false && $this->getEndDate() != ''
		...
		if ( $this->getDeleted() == true ) {
			...
			if ( $pplf->getRecordCount() > 0 ) {
				...
				if ( $pp_obj->isFirstPayPeriodInYear() == true
						&& time() >= $pp_obj->getStartDate() //Can't be end or transaction date, as those are too late. This helps prevent manual pay periods created in the future from triggering the maintenance.
						&& time() <= $this->getEndDate() //In cases of modifying old pay periods, make sure we aren't past the transaction date of the first pay period in the year.
				) {
			...
			if ( $this->getEnableImportData() == true && $this->getStatus() == 10 ) { //Only consider open pay periods.
				$ppslf = TTnew( 'PayPeriodScheduleListFactory' ); /** @var PayPeriodScheduleListFactory $ppslf */
