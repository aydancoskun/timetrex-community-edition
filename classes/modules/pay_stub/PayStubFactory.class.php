...
class PayStubFactory extends Factory {
	...
	function Validate( $ignore_warning = true ) {
		...
		if ( $this->getDeleted() == false ) {
			...
			if ( $this->getCurrency() !== false ) {
				...
				if ( $this->Validator->isError( 'currency' ) == false ) {
					if ( $culf->getRecordCount() == 1 && ( $this->isNew() || $this->old_currency_id != $this->getCurrency() ) ) {
		...
		if ( $this->getDeleted() == false && $this->isUniquePayStubType() == false ) {
	...
	function postSave() {
		...
		if ( $this->getEnableEmail() && $this->getStatus() == 40 && $this->isDataDifferent( 'status_id', $data_diff ) == true ) { //Paid
			$this->emailPayStub();
		} else {
	...
	function getEmailPayStubAddresses() {
		...
		if ( $uplf->getRecordCount() > 0 ) {
			foreach ( $uplf as $up_obj ) {
				if ( $up_obj->getEnableEmailNotificationPayStub() == true && is_object( $up_obj->getUserObject() ) && $up_obj->getUserObject()->getEnableLogin() == true ) {
