...
if ( $object_type != 'primary_company_logo' && $object_type != 'copyright' && $object_type != 'smcopyright' && $object_type != 'copyright_wide' ) {
...
switch ( $object_type ) {
				...
				if ( isset( $result[0] ) && count( $result[0] ) > 0 ) {
					...
					if ( $drlf->getRecordCount() == 1 ) {
						...
						if ( file_exists( $file_name ) ) {
							...
							$params['ContentDisposition'] = [ HTTP_DOWNLOAD_ATTACHMENT, basename( $dr_obj->getRemoteFileName() ) ];
							$params['cache'] = false;
						} else {
		...
		if ( $rl->check() == false ) {
			...
			if ( $cplf->getRecordCount() == 1 ) {
				...
				if ( file_exists( $file_name ) ) {
					...
					$params['ContentDisposition'] = [ HTTP_DOWNLOAD_ATTACHMENT, 'signature.png' ];
					$params['cache'] = false;
				} else {
					Debug::text( 'Document Downloads Failed! Attempt: ' . $rl->getAttempts(), __FILE__, __LINE__, __METHOD__, 10 );
					sleep( ( $rl->getAttempts() * 0.5 ) );
	...
	case 'invoice_config':
		...
		if ( file_exists( $file_name ) ) {
			...
			$params['ContentDisposition'] = [ HTTP_DOWNLOAD_INLINE, basename( $file_name ) ];
			$params['cache'] = true;
		}
		break;
	case 'company_logo':
	...
	case 'user_photo':
		...
		if ( $rl->check() == false ) {
			...
			if ( $permission->Check( 'user', 'view' )
				...
				$result = $api_f->stripReturnHandler( $api_f->getUser( [ 'filter_data' => [ 'id' => $object_id ] ] ) );
				if ( isset( $result[0] ) && count( $result[0] ) > 0 ) {
	...
	case 'copyright':
		...
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
			...
			if ( $file_name != '' && file_exists( $file_name ) ) {
				...
				$params['ContentDisposition'] = [ HTTP_DOWNLOAD_ATTACHMENT, 'pro_copyright.jpg' ];
				$params['data'] = file_get_contents( $file_name );
				$params['cache'] = true;
			}
		} else {
			$params['ContentType'] = 'image/jpeg';
			$params['ContentDisposition'] = [ HTTP_DOWNLOAD_ATTACHMENT, 'copyright.jpg' ];
	...
	case 'smcopyright':
		...
		if ( getTTProductEdition() >= TT_PRODUCT_PROFESSIONAL ) {
			...
			if ( $file_name != '' && file_exists( $file_name ) ) {
				$params['file'] = $file_name;
				$params['ContentType'] = 'image/png';
				$params['ContentDisposition'] = [ HTTP_DOWNLOAD_ATTACHMENT, 'pro_copyright_wide.png' ];
				$params['data'] = file_get_contents( $file_name );
				$params['cache'] = true;
			}
		} else {
			$params['ContentType'] = 'image/png';
			$params['ContentDisposition'] = [ HTTP_DOWNLOAD_ATTACHMENT, 'copyright_wide.png' ];
